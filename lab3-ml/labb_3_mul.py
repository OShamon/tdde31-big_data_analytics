...
def gaussian_kernel_date(date_1, date_2, h_date):
    ...
    d1 = days_in_month[int(date_1[5:7]) - 1] + int(date_1[8:10])
    d2 = days_in_month[int(date_2[5:7]) - 1] + int(date_2[8:10])
    diff = abs(d1 - d2)
    if diff > 183:
        diff = 366 - diff
...
stations = lines_station.map(lambda x: (x[0], gaussian_kernel_dist((float(x[3]), float(x[4])), a, b, h_distance)))
stations_b = sc.broadcast(stations.collect())
#temperature_readings_lon_lat = lines_temp.map(lambda x: (x[0], x[1], x[2], float(x[3])))
temperature_readings_lon_lat = lines_temp.filter(lambda x: datetime(int(x[1][0:4]), int(x[1][5:7]), int(x[1][8:10])) <= datetime(int(date[0:4]), int(date[5:7]), int(date[8:10])))

temperature_readings_lon_lat = temperature_readings_lon_lat.map(lambda x: (x[0], gaussian_kernel_date(x[1], date, h_date), x[2], get_lon_lat(stations_b.value, x[0]), float(x[3])))
...
for time in ["00:00:00", "22:00:00", "20:00:00", "18:00:00", "16:00:00", "14:00:00", "12:00:00", "10:00:00", "08:00:00", "06:00:00", "04:00:00"]:
    temprature_readings = temperature_readings_lon_lat.map(lambda x: (x[0], x[3], x[1], gaussian_kernel_time(x[2], time, h_time), float(x[4])))
    temp_readings = temprature_readings.map(lambda x: (1 , (x[1]*x[2]*x[3]*x[4], x[1]*x[2]*x[3])))
