...
from math import radians, cos, sin, asin, sqrt, exp from datetime import datetime
...
stations = sc.textFile("data/stations.csv") temps = sc.textFile("data/temps.csv")
# Your code here
def gaussian_kernel_dist(lon1, lat1, lon2, lat2, h_distance):
    return np.exp(-((havresine(lon1, lat1, lon2, lat2)/h_distance)**2))
...
"12:00:00", "10:00:00", "08:00:00", "06:00:00", "04:00:00"]:
    temprature_readings = lines_temp.map(lambda x: ((x[0], x[1][0:4], x[1][5:7], x[1][8:10]), gaussian_kernel_dist(x[0], stations_b.value, a, b)*float(x[3]), gaussian_kernel_date(x[1], date)*float(x[3]), gaussian_kernel_time(x[2], time)*float(x[3]))
